...
namespace Zappy {
  ...
  Config::Config(toml::table & table, const std::string language):
    language_acronym_(language)
   {
    ...
    for (std::vector<std::string>::const_iterator it = DIRECTIVES.begin();
  ...
  const std::string & Config::get(const std::string & directive) const {
    return toml_contents_.at(directive);
  }
  
  const std::string & Config::get_language_acronym() const {
    return (language_acronym_);
  }
  ...
  const std::string Config::get_value_or_empty(toml::table & table, const std::string language,
