...
namespace Zappy {
	...
	Join::Join(GameEngine *trantor, Client &c, std::string team):
		PlayerClientCommand(trantor, "join", 0), client_(c), team_(team), joined_(false) {
	}
	...
	void	Join::broadcast() const {
		...
		if (!is_valid() || !joined_)
			return ;
		Player *player = client_.get_player();
		ss << "pnw " <<
			player->get_id() << " " <<
			player->get_position() << " " <<
    ...
    if (joined_) {
			...
			ss << client_.get_player()->get_team().slots_left() << std::endl << std::string(trantor_->get_map_size());
